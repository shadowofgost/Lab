...
class IndexHandler(RequestHandler):
    ...
    def post(self):
        ...
        self.render("select.html",result_return=result,print_results=print_results,data ={"data":','.join(result_return)})
        ...
        for i in range(prize_number):
            ...
            if sign_random < count:
                if int(prize_number_list[i]) > 0:
                    sign = 1
                    result_return.append(name +";"+ str(i+1)+"等奖")
                    print_results = name+"获得"+str(i+1)+"等奖"
