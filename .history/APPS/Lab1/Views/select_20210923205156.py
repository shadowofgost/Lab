...
class IndexHandler(RequestHandler):
    ...
    def post(self):
        ...
        self.set_cookie("prize_number_list", prize_number_list.join(","))
        self.set_cookie("prize_random_list",prize_random_list.join(","))
        self.set_cookie("result_return",dumps(result_return))
        self.render("select.html",result_return=result_return,print_results=print_results)

class SelectHandler(RequestHandler):
    def get(self):
        self.render("select.html")
    def post(self):
        prize_number = loads(self.get_cookie("prize_number"))
        prize_number_list = loads(self.get_cookie("prize_number_list"))
        prize_random_list = loads(self.get_cookie("prize_random_list"))
        result_return = loads(self.get_cookie("result_return"))
        name = self.get_argument("name")
        sign = random()
        count = 0
        sign = 0
        for i in range(prize_number):
            count += prize_random_list[i]
            if sign < count:
                if prize_number_list[i] > 0:
                    sign = 1
                    result_return[name] = str(i)+"等奖"
                    print_results = name+"获得"+str(i)+"等奖"
                    prize_number_list[i] -= 1
                    break
        if sign == 0:
            result_return[name] = "未中奖"
            print_results = "未中奖"
        self.set_cookie("prize_number", dumps(prize_number))
        self.set_cookie("prize_number_list", dumps(prize_number_list))
        self.set_cookie("prize_random_list",dumps(prize_random_list))
        self.set_cookie("result_return",dumps(result_return))
        self.render("select.html",result_return=result_return,print_results=print_results)
